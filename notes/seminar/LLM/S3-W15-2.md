...
- MMQA
  ...
  - MMQA construction
    ...
    - Question Answer Annotation
      - Spider 에서 샘플을 추출한 다음, Pal et al. (2023) 의 패러다임에 따라 다중 테이블 SQL 쿼리를 합성
        - 구체적으로, 연구자들이 메뉴얼에 맞춰 직접 제작한 45개 쿼리 템플릿 + operation types 에 대한 수작업 규칙으로 SQL 쿼리를 제작
      - 이렇게 얻은 SQL 쿼리를 테이블과 함께 LLM (e.g., GPT-4-turbo) 에게 프롬프트하여 자연어 질문을 생성
      - 마지막으로, CS 분야 background 를 가진 2명의 전문가가 각 테이블의 PK 와 FK 를 주석처리
        - 2명의 전문가는 저자들이 확보한 SQL 쿼리+LLM 이 생성한 질문을 바탕으로 answer annotation 도 진행
