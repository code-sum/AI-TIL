...
- 3줄 요약
  - LLM 의 발전으로 멀티모달 LLM(MLLM) 이 범용 비서로서의 역량을 크게 키웠으나 여전히 사용자 맞춤형 지식이 부족하여 일상생활 활용에 제한이 있는바, 본 논문에서는 MLLM 의 개인화를 위한 '검색 증강 개인화(RAP)' 프레임워크를 제안
  - RAP는 외부 키-값 데이터베이스를 통해 사용자 정보를 기억(Remember)하고, 대화 시작 시 관련 정보를 검색(Retrieve)하여 맞춤형 답변을 생성(Generate)하는 3단계 과정을 거치며, 기존 방식과 달리 데이터베이스 업데이트를 통해 실시간으로 개념을 수정
  - 나아가 모델의 생성 품질을 높이기 위해 전용 데이터셋을 구축하고 학습시킨 RAP-MLLM은 대규모 사전 학습 덕분에 추가적인 미세조정(Finetuning) 없이도 무한한 시각적 개념으로 확장 가능하며, 개인화된 이미지 캡셔닝이나 질의응답 등 다양한 작업에서 뛰어난 유연성과 성능을 보여줌
