...
- Motivation
  ...
  - Network 분야 LLM 도입 시 도전 과제
    - (1) 입력 모달리티 격차: 텍스트 이외 데이터 적합성 부족
    - (2) 토큰 기반 답변 생성의 비효율성·실효성(오토리그레시브 예측의 지연, 잘못된 답변 발생 가능)
    - (3) 대규모 사전학습모델의 도메인 적응 비용(특히 RL 기반 작업 환경 상호작용 및 파라미터 크기 등)
...
- NetLLM 설계: 3가지 모듈로 구성
  ...
  - (3) DD-LRNA(Data-driven Low-Rank Networking Adaptation): 데이터 기반 적응 파이프라인 및 저차원 행렬로 파라미터 미세 조정(적은 학습 비용/메모리 소모)
    - LLM의 fine-tuning 비용을 획기적으로 줄이기 위한 scheme. 다음의 두 가지 핵심 디자인으로 구성
        ...
        - ② Low-rank adaptation
  - LLM 파라미터는 동결한 상태에서 위 3가지 모듈(인코더/헤드/저차원행렬)만 미세 조정
